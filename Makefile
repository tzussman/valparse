...
format:  ## run autoformatting with black
	python -m black valparse/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./valparse

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v valparse/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v valparse/tests --cov=valparse --cov-branch --cov-fail-under=75 --cov-report term-missing
